            ...
            return ( <
                    ...
                    Text style = { styles.townName } > { item.Name } < /Text> {
                        ...
                        ( <
                            ...
                            <
                            Text style = {
                                [styles.textDark, { marginBottom: 5, fontSize: 14 }] } > { 'No trash bag pickup locations in this town' } <
                            /Text> <
                            /View>
                        ) :
                        item.PickupLocations.map((loc, i) => ( <
                                    ...
                                    style = { styles.location } > {
                                        Boolean(loc.PickupLocationName) ? ( <
                                            Text style = {
                                                [styles.textDark, { marginBottom: 5, fontSize: 14 }] } > { loc.PickupLocationName.replace(newLineRegex, ' ').replace(/\s\s/g, ' ') } <
                                            /Text>) : null} {
                                                Boolean(loc.PickupLocationAddress) ? ( <
                                                        Text style = {
                                                            [styles.textDark, { marginBottom: 5, fontSize: 14 }] } > { loc.PickupLocationAddress.replace(newLineRegex, ' ').replace(/\s\s/g, ' ') } <
                                                        /Text>) : null} {
                                                            Boolean(loc.PickupNotes) ? ( <
                                                                Text style = {
                                                                    [styles.textDark, { marginBottom: 0, fontSize: 14 }] } > { loc.PickupNotes.replace(newLineRegex, ' ').replace(/\s\s/g, ' ') } <
                                                                /Text>) : null} <
                                                                /View>
                                                            ))
                                                    } <
                                    ...
                                    class TrashBagFinder extends Component {
                                        ...
                                        render() {
                                                ...
                                                return ( <
                                                    ...
                                                    /> <
                                                    /View> <
                                                    KeyboardAvoidingView style = { defaultStyles.frame }
                                                    ...
                                                    <
                                                    FlatList style = { styles.infoBlockContainer }
                                                    data = { locations }
                                                    renderItem = {
                                                        ({ item }) => ( < TownItem item = { item }
                                                            />)}/ >
                                                            <
                                                            /View> <
                                                            /ScrollView> <
