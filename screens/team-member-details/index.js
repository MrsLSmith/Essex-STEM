...
const TeamMemberDetails = ({ actions, messages, navigation, teamMembers, teams, currentUserId }: PropsType): React$Element<any> => {
    ...
    const updateTeamMember = (newStatus: Object): (any=>void) => () => {
        const messageIds = Object.keys(messages).filter((id: string): boolean => messages[id].teamId === teamId && messages[id].type === "REQUEST_TO_JOIN" && messages[id].sender.uid === member.uid);
        const _member = TeamMember.create(Object.assign({}, member, (newStatus ? { memberStatus: newStatus } : {})));
        messageIds.forEach((id: string) => {
            actions.deleteMessage(currentUserId, id);
        });
        navigation.goBack();
                ...
                },
                {
                    text: "Yes", onPress: () => {
                        const messageIds = Object
                            .keys(messages)
                            .filter((id: string): boolean => messages[id].teamId === teamId && messages[id].type === "REQUEST_TO_JOIN" && messages[id].sender.uid === member.uid);
                        messageIds.forEach((id: string) => {
                            actions.deleteMessage(currentUserId, id);
                        });
                        navigation.goBack();
                        actions.removeTeamMember(teamId, member);
                    }
                }
            ],
            { cancelable: true }
    ...
    const getButtons = (teamMember: Object = {}): ?React$Element<any> => {
    ...
    const getStatus = (teamMember: Object = {}, _isOwner: boolean): React$Element<any> => {
        switch (teamMember.memberStatus) {
            case status.OWNER :
                return (
...
TeamMemberDetails.navigationOptions = {
    title: "User Profile"
};
